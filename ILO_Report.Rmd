---
title: "ILO summary- Week 1 Report"
...
no_ilo_plus_1 <- ilo_data %>% select(Course, ILO) %>% distinct() %>% nrow()
```
#Abstract
This document presents a brief overview of the current state of the *Intended Learning Objectives* (hereafter *ILO*). Included is information on:  
1. The orientation of language used in ILOs  
2. The most common objectives broken down by Level and Concentration  
3. A description of how the objectives map to Bloom's Taxonomy of learning  
4. Recommendations  

#Introduction *A brief overview of our data*
...
The data contained a total of **`r no_courses`** **courses**, which amounted to **`r no_ilo_plus_1 -1`** **ILOs**.
```{r clean workspace 1}
rm(no_courses,no_ilo_plus_1)
```

##Distribution of ILO per course
In general, the courses had an average of `r round(no_ilo_plus_1/no_courses,1)` *ILOs* per course. Which were distributed as follows:
...
* Student Oriented *ILO*: ""().  
* Course Oriented *ILO*: ""().

Some courses had *ILOs* formulated in both a student and a course oriented way. 
...
verbs_in_ilos <- annotated_ilo %>%
  filter(upos == "VERB") 
```
Universal Verb counts
```{r verb counts basic text analysis}
...
```
Lets see what I ended up with:
```{r view clean ilos}
ilo_data_clean %>% View()

```

Time for some summary graphs and summary statistics:
###Summary Statistics
####Total verbs in ILOs:
```{r}
total_objectives <- ilo_data_clean %>% group_by(ILO) %>% mutate(Verbs_per_ILO = n()) %>%
  
# for verification join with:
# %>% arrange(Course, ILO) %>% select(Course, Verbs_per_ILO, Verb, ILO, everything())
  ungroup() %>%
  ...
  select(Course, verbs_per_course) %>%
  distinct() %>%
  ungroup() %>%
  summarize(total = sum(verbs_per_course)) %>%
  pull(total)

total_objectives
```
we have 856 verbs (objectives).
###Graphs
####Universal Verbs
Universal use of verbs:
```{r verb counts maunal text analysis}
...
verbs_unrepeated        %>%
  ...
  geom_histogram(stat = "identity", fill = "navy") +
  labs(title = "Most Common Verbs across all Courses", x = "Verb", y = "Number of appearances") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

####Breakdown by Level
```{r level break down manual text analysis}
verbs_unrepeated <- ilo_data_clean %>%
  group_by(Level, Verb)            %>%
  mutate(n = n())                  %>%
  select(Level, Verb, n)           %>%
