...
cognitive_process_tmp <- cognitive_process_unrepeated %>%
  ...
  distinct()   %>%
  mutate(Order = case_when(Cognitive_Process == "Remember"~ 1,
                           Cognitive_Process == "Understand"~ 2,
                           Cognitive_Process == "Apply"~ 3,
                           Cognitive_Process == "Analyse"~ 4,
                           Cognitive_Process == "Evaluate"~ 5,
                           Cognitive_Process == "Create"~ 6
                           ))

cognitive_process_tmp        %>%
  ggplot(aes(x = reorder(Cognitive_Process, Order), y = n)) +
  geom_histogram(stat = "identity", fill = "navy") +
...
cognitive_process_tmp <- cognitive_process_unrepeated %>%
  ...
  mutate(Order = case_when(Cognitive_Process == "Remember"~ 1,
         ...
         Cognitive_Process = as.factor(Cognitive_Process))
...
cognitive_process_tmp <- cognitive_process_unrepeated %>%
  ...
  mutate(Order = case_when(Cognitive_Process == "Remember"~ 1,
         ...
         Cognitive_Process = as.factor(Cognitive_Process)) # need to be factor in order not to drop.
...
cognitive_process_tmp <- cognitive_process_unrepeated %>%
  ...
  mutate(n = n()) %>%
  ungroup() %>%                        # This part is to avoid dropping in plot
  mutate(Level = as.factor(Level) )%>% # This part is to avoid dropping in plot
...
for (s_student in student_sample){
  plot <- student_bloom_profile %>% 
    group_by(`Student ID`) %>%
    ...
    ggplot(aes(x = reorder(Cognitive_Process, Order), y = Value))+
    geom_bar(stat = "identity", fill = "navy") +
    labs(title= paste("Cognitive Processes for", s_student, "accross all their courses", sep = " "), x = "Cognitive Process", y = "Number of ILO's") +
    theme_minimal()
   print(plot)
}
...
I'm missing the ILOs of the following UCM courses (they appear in transcripts):

```{r missing}
collapse_or_2 <- function(string_vector){
  paste(string_vector, collapse ="|")
}

courses_with_ilo_data <- ilo_data %>% select(Course) %>% distinct
UCM_codes <- courses_with_ilo_data %>% transmute(strings = str_sub(Course,1, 3)) %>% unique() %>% pull()
UCM_codes <- c(UCM_codes, "CAP")


all_courses <- d_transcript %>% select(`Course ID`) %>% distinct() %>% arrange(`Course ID`)

no_ilo_courses <- all_courses %>% anti_join(courses_with_ilo_data, by = c("Course ID" ="Course")) 
no_ilo_ucm_courses <- no_ilo_courses %>% mutate(is_UCM = str_detect(`Course ID`, collapse_or_2(UCM_codes))) %>% filter(is_UCM == TRUE)
```
Total: 

```{r print missing}
print(dim(no_ilo_ucm_courses %>% select(`Course ID`))[1])

knitr::kable(no_ilo_ucm_courses %>% select(`Course ID`))
```
## Missing from ILO file with cluster:

The following courses from the cluster groups are missing from my ILO file:

```{r missing from ilos in cluster file}
# Courses included in ILO file 
test_courses_w_ilos<- ilo_data %>% select(Course) %>% unique() %>% mutate(ILO = T)

# Courses in cluster data

test_courses_w_cluster <- cluster_data %>% mutate(has_Cluster = T)

# Test
test_join <- test_courses_w_cluster %>% full_join(test_courses_w_ilos, by = "Course")

# Couses in cluster with no ilos
with_cluster_without_ilo <- test_join %>% filter(is.na(ILO)) %>% unique()
with_ilo_without_cluster <- test_join %>% filter (is.na(has_Cluster)) %>% unique()



# Courses in clusters with no ILO
# cluster_data %>% left_join(no_ilo_ucm_courses, by = c("Course" = "Course ID")) %>% filter(!is.na(is_UCM)) %>%
# select(-is_UCM)
# Courses with no ilo and no cluster
# cluster_data %>% inner_join(no_ilo_ucm_courses, by = c("Course" = "Course ID")) %>% select(-is_UCM)

knitr::kable(with_cluster_without_ilo %>% select(Course, Cluster))

```

## Missing from Cluster File
The following courses have ILO data but have not been assigned a cluster (WARNING, THE COURSES ARE PROBABLY DISCONTINUED THIS YEAR):

```{r missing cluster}

knitr::kable(with_ilo_without_cluster %>% select(Course))
```
