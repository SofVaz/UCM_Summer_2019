...
#Methods in ILO's
```{r}
no_ilo_methods <- ilo_data_clean %>% filter(`Methodology LO` == "M") %>% nrow()
```

A separate question to consider is: how many of our ILOs explicitly refer to methodology?
During the verb extraction, a comment was also made refering to whether or not the ILO explicitly refered to methodology. Out of the `r no_ilo` ILOs existing in the curricula, there were `r no_ilo_methods` about methodoloy (this is `r 100*no_ilo_methods/no_ilo`%) that refer.Here is a summary of that:
```{r}
ilo_data_clean %>% filter(`Methodology LO` == "M")
```
