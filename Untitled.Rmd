...
We notice that some of the words recognized by are not quite equivalent. We create equivalent classes:
...
ilo_data_clean %>% mutate(
  ...
  Verb = map_chr(Verb, function(string) str_replace(string, "^na$", NA_character_))
  ) %>%
  distinct() %>% arrange(Verb) %>% View()
