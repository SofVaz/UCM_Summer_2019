...
```{r clean verbs data}
ilo_data_clean <- ilo_data %>% gather(key = tmp_place, value = Verb, Verb_used_1:Verd_used_3) %>% select(-tmp_place)

dirty_verbs <- ilo_data_clean %>% pull(Verb) %>% unique()
dirty_verbs
```
as you can see some are really similar, for example, we have: "to understand", "understand" and "basic understanding". Let's group all of these together.
```{r collapse or}
...
```{r cleaning}
#UNDERSTAND
understand_equivalence <- ilo_data_clean %>% filter(str_detect(Verb, "understa")) %>% pull(Verb) %>% unique()
print(paste('The verbs that were taken to be the same as "understand" are:', paste(understand_equivalence, collapse = ", "), sep = " "))

#KNOW
know_equivalence <- ilo_data_clean %>% filter(str_detect(Verb, "know")) %>% pull(Verb) %>% unique()
...
#[GAIN] FAMILIARITY
familiarize_equivalence <- ilo_data_clean %>% filter(str_detect(Verb, "familia")) %>% pull(Verb) %>% unique()
print(paste('The verbs that were taken to be the same as "[gain] familiarity" are:', paste(familiarize_equivalence, collapse = ", "), sep = " "))


```
NOTES:"gain basic practial knowledge" and "use knowledge" might not be correct here rather in "apply".
```{r cleaning other verbs}
standardize_knowing <- function(string){
  str_replace(string, "to know|to have knowledge|basic knowledge", "know")
}

standardize_understand <- function(string){
  str_replace(string, "to understand|to develop understanding|gain understanding|basic understanding", "understand")
}

standardize_familiarity <- function(string){
  str_replace(string, "to familiarize|to develop understanding|gain understanding|basic understanding", "understand")
}
...
ilo_data_clean %>% transmute(Verb_used_1 = map_chr(Verb_used_1, standardize_knowing),
                             Verb_used_1 = map_chr(Verb_used_1, standardize_understand)) %>%
  distinct()
