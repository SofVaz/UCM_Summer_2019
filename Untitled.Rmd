...
```{r annotate data}
annotated_data <- ilo_word_counts %>% left_join(x, by = c("word" = "token"))
```
Filtering for verbs:
...
verbs_unrepeated[1:20,] %>% ggplot(aes(x = reorder(word,-n), y = n))+
  geom_histogram(stat = "identity", fill = "navy")+
...
data_breakdown %>% map(data, )<- basic_word_counts %>% left_join(x, by = c("word" = "token")) %>%
  filter(upos == "VERB")
